...
public:
    ...
    //
    // Pass the model reference up to the parent constructor
    Evokilo5(ModelPosition *_pos, Settings *_settings,
             std::vector<std::string> _words, std::string _logfile = "") :
    Kilobot (_pos, _settings),
    words   (_words),
    logfile (_logfile),
    ...
    inputs  (3)
    {
        ...
        kilo_message_tx         = (message_tx_t)&Evokilo5::message_tx_dummy;
        kilo_message_rx         = (message_rx_t)&Evokilo5::message_rx_dummy;
        kilo_message_tx_success = (message_tx_success_t)&Evokilo5::message_tx_success_dummy;
        setup();
    }
    ~Evokilo5()
    {
